...
Common options:
  ...
  --wait                     Wait for the page bridge instead of failing (connect; see --wait-timeout)
...
async function execute(positionals, options) {
  ...
  if (command === 'init') {
    const target = normalizePath(options.project || options.dir || process.cwd(), { cwd: process.cwd() });
    const name = String(options.name || path.basename(target) || 'My Userscript');
    const match = String(options.match || 'https://example.com/*');
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'my-userscript';
  ...
  if (command === 'connect') {
    ...
    if (options.wait && !status.health.browser.connected) {
      ...
      while (Date.now() < deadline && !status.health.browser.connected) {
        ...
        if (!status.health) throw new VibeCatError('SERVICE_NOT_RUNNING', 'The VibeCat service stopped while waiting for the page bridge.', { retryable: true, nextActions: ['Run `vibecat start --json`.'] });
      }
    }
    if (!status.health.browser.connected) throw new VibeCatError('BROWSER_NOT_CONNECTED', 'No browser tab has acknowledged the authenticated VibeCat bridge.', { evidence: { activeSessions: status.health.browser_sessions }, retryable: true, nextActions: options.wait ? ['Open or reload the matched URL in the synchronized browser, then rerun `vibecat connect --wait --json` (or raise `--wait-timeout`).'] : ['Load or reload the synchronized userscript in the intended tab.', 'Rerun `vibecat connect --json`.'] });
